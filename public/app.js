(function() {
    ...
    const API = {
        ...
        async getStats() {
            try {
                const response = await fetch('/api/measurements/stats', { credentials: 'include' });
                if (response.ok) {
                    return await response.json();
                }
                return null;
        ...
        async getTodaySession() {
            try {
                const response = await fetch('/api/daily-sessions/today', { credentials: 'include' });
                if (response.ok) {
                    return await response.json();
                }
                return { totalDurationSeconds: 0, sessionCount: 0 };
            } catch (error) {
        ...
        async getTodayMeasurements() {
            try {
                const response = await fetch('/api/measurements/today', { credentials: 'include' });
                if (response.ok) {
                    return await response.json();
                }
                return [];
        ...
        async getLastSession() {
            try {
                const response = await fetch('/api/daily-sessions/last', { credentials: 'include' });
                if (response.ok) {
                    return await response.json();
                }
                return null;
    ...
    function updateVoiceToggleUI() {
        ...
        if (AppState.isVoiceEnabled) {
            toggle.className = 'p-2 rounded-lg bg-success text-white';
        } else {
            toggle.className = 'p-2 rounded-lg bg-gray-100 text-gray-600';
        }
    }
    ...
    async function updateUserInterfaceAuth() {
        if (AppState.currentUser) {
            // Show logged in state - no login prompt to hide since it was removed
            
            // Update header button - Show only profile image
            const profileImageUrl = AppState.currentUser.profileImageUrl || 'data:image/svg+xml;base64,';
            
            elements.userAccountBtn.innerHTML = `
                <img src="${profileImageUrl}" alt="프로필" style="width: 32px; height: 32px; border-radius: 50%; border: none; outline: none; box-shadow: none; object-fit: cover;">
            `;
            elements.userAccountBtn.className = '';
            elements.userAccountBtn.removeAttribute('style');
            elements.userAccountBtn.style.cssText = 'border: none; outline: none; box-shadow: none; background: transparent; padding: 0;';
            
            // Update dropdown content
            elements.dropdownName.textContent = AppState.currentUser.name || '사용자';
            elements.dropdownEmail.textContent = AppState.currentUser.email || '';
            ...
            // Show not logged in state - no login prompt to show since it was removed
            ...
            `;
            elements.userAccountBtn.className = 'flex items-center space-x-2 p-2 rounded-lg bg-primary text-white';
            elements.userAccountBtn.removeAttribute('style');
        }
    }
    ...
    async function openSettingsModal() {
        ...
        if (settings) {
            elements.settingsNotificationToggle.checked = settings.notifications;
            elements.settingsAlarmTime.value = settings.alarmTime;
        } else {
            // Set default values
            elements.settingsNotificationToggle.checked = true;
            elements.settingsAlarmTime.value = '07:00';
        }
    ...
    async function saveUserSettings() {
        ...
        const settings = {
            notifications: elements.settingsNotificationToggle.checked,
            alarmTime: elements.settingsAlarmTime.value
        };
    ...
    function initEventListeners() {
        ...
        const handleVoiceToggle = () => {
            ...
            if (AppState.isVoiceEnabled) {
                toggle.className = 'w-full flex items-center space-x-3 p-3 bg-green-100 hover:bg-green-200 text-green-700 rounded-lg border border-green-300 transition-all duration-200';
                iconDiv.className = 'w-8 h-8 bg-green-200 rounded-full flex items-center justify-center';
                icon.setAttribute('class', 'w-4 h-4 text-green-700');
                VoiceManager.speak('음성 안내가 켜졌습니다');
            } else {
                toggle.className = 'w-full flex items-center space-x-3 p-3 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg border border-gray-200 transition-all duration-200';
                iconDiv.className = 'w-8 h-8 bg-gray-200 rounded-full flex items-center justify-center';
                icon.setAttribute('class', 'w-4 h-4 text-gray-600');
            }
        };
        ...
        addEventHandler(document.getElementById('requestPermissionBtn'), handleRequestPermission);
        addEventHandler(document.getElementById('closePermissionModal'), handleClosePermission);
        addEventHandler(document.getElementById('closeCompletionModal'), handleCloseCompletion);
        ...
        addEventHandler(elements.userAccountBtn, handleUserAccountBtn);

        // Close dropdown when clicking outside
        document.addEventListener('click', (e) => {
            if (!elements.profileDropdown.contains(e.target) && !elements.userAccountBtn.contains(e.target)) {
                elements.profileDropdown.classList.add('hidden');
            }
        });
        ...
        addEventHandler(elements.googleLoginBtn, handleGoogleLogin);
        addEventHandler(elements.kakaoLoginBtn, handleKakaoLogin);
        addEventHandler(document.getElementById('closeLoginModal'), handleCloseLoginModal);
        addEventHandler(elements.logoutBtn, handleLogout);
